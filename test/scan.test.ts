...
import { discoverFiles } from '../src/core/discoverFiles';
import { getLanguage } from '../src/core/language';
import { countLoc } from '../src/core/locCounter';
...
describe('Codeworth Core', () => {
    ...
    it('should discover files respecting gitignore', async () => {
        ...
        const relativePaths = files.map(f => path.relative(path.resolve(FIXTURE_DIR), path.join(path.resolve(FIXTURE_DIR), f))); // discoverFiles returns relative paths, but let's be safe
