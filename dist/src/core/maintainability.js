...
exports.calculateMaintainability = calculateMaintainability;
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const complexityProxy_js_1 = require("./complexityProxy.js");
const scoringChecks_js_1 = require("./scoringChecks.js");
// Weights
const MAX_SCORES = {
    HYGIENE: 20,
    STRUCTURE: 25,
...
async function calculateMaintainability(scanResult, rootDir) {
    const rootFiles = await promises_1.default.readdir(rootDir).catch(() => []);
    const hygiene = await runChecks('Project Hygiene', scoringChecks_js_1.hygieneChecks, rootFiles, rootDir, MAX_SCORES.HYGIENE);
    const structure = runStructureChecks(scanResult, rootFiles);
    const complexity = runComplexityChecks(scanResult);
    const testing = await runChecks('Testing', (0, scoringChecks_js_1.testingChecks)(scanResult), rootFiles, rootDir, MAX_SCORES.TESTING);
...
function runComplexityChecks(scanResult) {
    const repoComplexity = (0, complexityProxy_js_1.calculateRepoComplexity)(scanResult);
...
async function runDependencyChecks(rootDir, rootFiles) {
    ...
    try {
        const pkg = JSON.parse(await promises_1.default.readFile(path_1.default.join(rootDir, 'package.json'), 'utf-8'));
