...
const gitChurn_js_1 = require("../../core/gitChurn.js");
const maintainability_js_1 = require("../../core/maintainability.js");
const quickWins_js_1 = require("../../core/quickWins.js");
const valuation_js_1 = require("../../core/valuation.js");
const directoryStats_js_1 = require("../../core/directoryStats.js");
const writeJson_js_1 = require("../output/writeJson.js");
const writeMarkdown_js_1 = require("../output/writeMarkdown.js");
...
function registerScanCommand(program) {
    program
        ...
        .addOption(new commander_1.Option('--preset <preset>', 'Rate preset (overrides rate)').choices(['junior', 'senior', 'agency']))
        .option('--exclude <patterns...>', 'Glob patterns to exclude')
        .action(async (options) => {
        try {
            const startTime = Date.now();
            console.log(chalk_1.default.blue(`Scanning ${options.path}...`));
            const scanOptions = parseOptions(options);
            // 1. Discover
            const filePaths = await collectScanContext(scanOptions);
            console.log(chalk_1.default.gray(`Found ${filePaths.length} files.`));
            // 2. Analyze
            const context = await analyzeProject(filePaths, options);
            // 3. Aggregate
            const partialResult = await buildPartialResult(context, options);
            // 4. Score & Value
            const result = await computeScores(partialResult, options, path_1.default.resolve(options.path));
            // 5. Report
...
async function collectScanContext(scanOptions) {
    return (0, discoverFiles_js_1.discoverFiles)(scanOptions);
}
async function analyzeProject(filePaths, options) {
    ...
    for (const filePath of filePaths) {
        const absolutePath = path_1.default.join(path_1.default.resolve(options.path), filePath);
        const lang = (0, language_js_1.getLanguage)(filePath);
        const loc = await (0, locCounter_js_1.countLoc)(absolutePath, lang);
        const size = (await promises_1.default.stat(absolutePath)).size;
    ...
    // Git Churn
    const isGitRepo = await (0, gitChurn_js_1.hasGit)(path_1.default.resolve(options.path));
    if (isGitRepo && !options.noGit) {
        console.log(chalk_1.default.gray('Analyzing git churn for top files...'));
        const sorted = [...files].sort((a, b) => b.loc.code - a.loc.code).slice(0, 200);
        for (const file of sorted) {
            file.churn = await (0, gitChurn_js_1.getFileChurn)(path_1.default.join(path_1.default.resolve(options.path), file.path), path_1.default.resolve(options.path));
        }
    }
...
async function buildPartialResult(context, options) {
    ...
    const directories = (0, directoryStats_js_1.computeDirectoryStats)(files);
    // Hotspots
    ...
    return {
        meta: {
            scannedPath: path_1.default.resolve(options.path),
            timestamp: new Date().toISOString(),
            repo: { name: path_1.default.basename(path_1.default.resolve(options.path)), hasGit: isGitRepo }
        },
        files,
        totals,
        languages,
...
async function computeScores(partialResult, options, scannedPath) {
    const maintainability = await (0, maintainability_js_1.calculateMaintainability)(partialResult, scannedPath);
    maintainability.quickWins = (0, quickWins_js_1.getQuickWins)(maintainability, partialResult);
...
function computeValuation(partialResult, maintainability, options) {
    ...
    if (options.preset === 'junior')
        ...
        locRate = 18;
    return (0, valuation_js_1.calculateValuation)(partialResult, maintainability, { locRateUsd: locRate });
}
async function generateOutputs(result, options) {
    const outDir = path_1.default.resolve(options.outdir);
    await promises_1.default.mkdir(outDir, { recursive: true });
    if (options.json) {
        const jsonPath = await (0, writeJson_js_1.writeJson)(result, outDir);
        console.log(chalk_1.default.green(`JSON report written to ${jsonPath}`));
    }
    if (options.md) {
        const mdPath = await (0, writeMarkdown_js_1.writeMarkdown)(result, outDir);
        console.log(chalk_1.default.green(`Markdown report written to ${mdPath}`));
    }
}
function printSummary(startTime) {
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    console.log(chalk_1.default.blue(`Scan completed in ${duration}s`));
}
