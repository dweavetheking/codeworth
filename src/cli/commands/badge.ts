...
import { evaluateCiGate, CiOptions } from '../../core/ci.js';

export function registerBadgeCommand(program: Command) {
    program
        .command('badge')
        ...
        .action(async (options) => {
            try {
                ...
                if (options.ci) {
                    ...
                    // 1. Locate Report
                    let reportPath = '';
                    let reportData: ScanResult | null = null;
                    ...
                    const ciResult = evaluateCiGate(reportData as ScanResult, ciOptions);
                    ...
                    const languagesMap = new Map<string, { files: number, loc: number }>();
