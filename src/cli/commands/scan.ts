...
import { ScanResult, FileResult, ScanOptions } from '../../core/types.js';
...
function parseOptions(options: any): ScanOptions {
    return {
        path: options.path,
...
async function collectScanContext(scanOptions: ScanOptions): Promise<string[]> {
    return discoverFiles(scanOptions);
}

async function analyzeProject(filePaths: string[], options: any) {
...
async function buildPartialResult(context: any, options: any): Promise<ScanResult> {
    const { files, totals, languageStats, isGitRepo } = context;

    const languages = Array.from(languageStats.entries()).map(([name, stats]: any) => ({
        name,
        files: stats.files,
        loc: stats.loc,
        percentOfCode: totals.loc.code > 0 ? (stats.loc.code / totals.loc.code) * 100 : 0
    })).sort((a: any, b: any) => b.loc.code - a.loc.code);

    const directories = computeDirectoryStats(files);

    // Hotspots
    const topN = options.top ? parseInt(options.top as string) : 20;
    const hotspots = {
...
async function computeScores(partialResult: ScanResult, options: any, scannedPath: string): Promise<ScanResult> {
...
function computeValuation(partialResult: ScanResult, maintainability: any, options: any) {
    let locRate = parseInt(options.rate);
    if (options.preset === 'junior') locRate = 6;
    else if (options.preset === 'senior') locRate = 12;
    else if (options.preset === 'agency') locRate = 18;

    return calculateValuation(partialResult, maintainability, { locRateUsd: locRate });
}

async function generateOutputs(result: ScanResult, options: any) {
