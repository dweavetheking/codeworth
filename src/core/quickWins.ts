import { MaintainabilityResult, ScanResult } from './types.js';

export function getQuickWins(maintainability: MaintainabilityResult, scanResult: ScanResult): string[] {
    const wins: string[] = [];

    // Hygiene wins
    const hygiene = maintainability.breakdown.find(c => c.category === 'Project Hygiene');
    if (hygiene && hygiene.score < hygiene.max) {
        if (!hygiene.notes.some(n => n.includes('README'))) wins.push('Add a README.md file.');
        if (!hygiene.notes.some(n => n.includes('LICENSE'))) wins.push('Add a LICENSE file.');
        // Only suggest CI if we are sure there is none and it wasn't filtered out
        if (!hygiene.notes.some(n => n.includes('CI'))) wins.push('Add CI configuration (GitHub Actions, etc).');
    }
    ...
    if (structure && structure.score < structure.max) {
        ...
        if (!structure.notes.some(n => n.includes('Linter'))) wins.push('Add eslint/prettier configuration.');
    }

    // Testing wins
    const testing = maintainability.breakdown.find(c => c.category === 'Testing');
    if (testing && testing.score < testing.max) {
        if (!testing.notes.some(n => n.includes('Tests directory'))) wins.push('Create a test directory and add tests.');
    }
