...
import { ScanResult, MaintainabilityResult } from './types.js';
import { calculateRepoComplexity } from './complexityProxy.js';
import { hygieneChecks, testingChecks, ScoreCheck } from './scoringChecks.js';
...
export async function calculateMaintainability(scanResult: ScanResult, rootDir: string): Promise<MaintainabilityResult> {
...
async function runChecks(category: string, checks: ScoreCheck[], files: string[], rootDir: string, max: number) {
    let score = 0;
    const notes: string[] = [];
...
function runStructureChecks(scanResult: ScanResult, rootFiles: string[]) {
    let score = 0;
    const notes: string[] = [];
    const max = MAX_SCORES.STRUCTURE;

    const hugeFiles = scanResult.files.filter(f => f.loc.code > 800);
    if (hugeFiles.length === 0) { score += 10; notes.push('No huge files'); }
    else if (hugeFiles.length < 5) { score += 5; notes.push(`Few huge files (${hugeFiles.length})`); }
    else notes.push(`Many huge files (${hugeFiles.length})`);
    ...
    if (rootFiles.includes('tsconfig.json')) { score += 5; notes.push('TypeScript detected'); }
...
function runComplexityChecks(scanResult: ScanResult) {
    const repoComplexity = calculateRepoComplexity(scanResult);
    let score = Math.round(25 * (1 - (repoComplexity / 100)));
    score = Math.max(0, Math.min(25, score));

    const notes: string[] = [];
    if (score > 20) notes.push('Low complexity');
    else if (score > 10) notes.push('Moderate complexity');
    else notes.push('High complexity');

    const extreme = scanResult.files.filter(f => (f.complexity || 0) > 85);
    if (extreme.length === 0) notes.push('No extreme hotspots');
    else notes.push(`${extreme.length} extreme hotspots`);

    return { category: 'Complexity', score, max: MAX_SCORES.COMPLEXITY, notes };
}

async function runDependencyChecks(rootDir: string, rootFiles: string[]) {
    let score = 0;
    const notes: string[] = [];
    ...
    try {
        ...
        if (deps < 20) { score += 5; notes.push('Low dependencies'); }
        else if (deps < 50) { score += 2; notes.push('Moderate dependencies'); }
    } catch {
        score += 2;
    }
