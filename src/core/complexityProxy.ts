...
export async function calculateComplexity(filepath: string): Promise<ComplexityScore> {
...
function analyzeContent(content: string): ComplexityScore {
...
function calculateLineNesting(line: string): number {
    const leadingSpace = line.match(/^\s*/)?.[0].length || 0;
    return Math.floor(leadingSpace / 2);
}

function countTokens(trimmedLine: string): { branches: number; functions: number } {
    const tokens = trimmedLine.split(/[^a-zA-Z0-9_]+/);
    let branches = 0;
    let functions = 0;

    for (const token of tokens) {
        if (BRANCH_KEYWORDS.has(token)) branches++;
        if (FUNC_KEYWORDS.has(token)) functions++;
    }
    return { branches, functions };
}

function computeRawScore(branches: number, functions: number, nesting: number, loc: number): number {
...
export function calculateRepoComplexity(scanResult: ScanResult): number {
    const complexFiles = scanResult.files.filter(f => f.complexity !== undefined);
    if (complexFiles.length === 0) return 0;
