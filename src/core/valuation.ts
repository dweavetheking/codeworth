...
export interface ValuationResult {
    ...
    assumptions: {
        rateModel: string; // 'per_loc' | 'hourly'
        locRateUsd: number;
        multipliers: {
            complexity: number;
            tests: number;
...
export function calculateValuation(scanResult: ScanResult, maintainability: MaintainabilityResult, options?: ValuationOptions): ValuationResult {
    // 1. Base Cost
    ...
    let mTests = 1.0;
    if (testScore >= 15) mTests = 1.10; // Good tests -> Higher asset value
    else if (testScore >= 8) mTests = 1.00;
    else if (testScore >= 1) mTests = 0.90;
    else mTests = 0.80; // No tests -> Penalty on value
    ...
    let mDeps = 1.0;
    if (depScore >= 8) mDeps = 1.05; // Low risk
    else if (depScore >= 5) mDeps = 1.0;
    else mDeps = 0.90; // High risk
    ...
    if (scanResult.meta.repo.hasGit) confidenceScore += 2; // Git is a strong signal
    if (testScore > 5) confidenceScore += 2; // Tests are a strong signal
    if (hygieneCat?.notes.some(n => n.includes('CI'))) confidenceScore += 1;
    if (scanResult.totals.files > 5) confidenceScore += 1;
    if (depCat && depCat.score >= 5) confidenceScore += 1; // Dependencies managed

    // Penalty
    if (scanResult.totals.loc.code < 100) confidenceScore -= 5; // Too small to judge
