...
export interface ScoreCheck {
    score: number;
    note: string;
    condition: (files: string[], rootDir: string) => boolean | Promise<boolean>;
}

export const hygieneChecks: ScoreCheck[] = [
...
export const testingChecks = (scanResult: ScanResult): ScoreCheck[] => [
    {
        score: 5,
        note: 'Tests directory exists',
        condition: async (_, root) => {
            return fs.stat(path.join(root, 'test')).then(() => true).catch(() =>
                fs.stat(path.join(root, 'tests')).then(() => true).catch(() => false));
        }
    },
    {
        score: 5,
